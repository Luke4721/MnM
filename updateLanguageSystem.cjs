...
// 1. Update LanguagePrompt.tsx
const newLanguagePrompt = `import React, { useState, useEffect } from 'react';
...
const LanguagePrompt: React.FC = () => {
  ...
  const handleSelectLanguage = (lang: string) => {
    ...
    if (lang === 'ru') {
      ...
      // If we are currently in RU, reload to clear it
...
`;
fs.writeFileSync('src/components/LanguagePrompt.tsx', newLanguagePrompt);

// 2. Update App.tsx
let appCode = fs.readFileSync('src/App.tsx', 'utf8');

// Ensure Languages icon is imported
if (!appCode.includes('Languages')) {
  appCode = appCode.replace(/import \{ Globe, Menu, X \} from 'lucide-react';/, "import { Globe, Menu, X, Languages } from 'lucide-react';");
}

// Add the Languages button next to the Globe button
const languageBtnStr = `
              <button
...
appCode = appCode.replace(
  /<button onClick=\{\(\) => setModalOpen\(true\)\} className="p-2 bg-gray-200\/50 dark:bg-white\/10 rounded-full text-gray-800 dark:text-white">\s*<Globe size=\{18\} \/>\s*<\/button>/,
...
appCode = appCode.replace(
  /<button\s*onClick=\{\(\) => setModalOpen\(true\)\}\s*className="p-2 bg-gray-200\/50 dark:bg-white\/10 rounded-full hover:bg-gray-300\/50 dark:hover:bg-white\/20 transition-colors text-gray-800 dark:text-white border border-gray-300\/50 dark:border-white\/10 cursor-pointer"\s*>\s*<Globe size=\{18\} \/>\s*<\/button>/,

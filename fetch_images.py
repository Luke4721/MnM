...
def get_wikimedia_image(query):
    ...
    url = f"https://en.wikipedia.org/w/api.php?action=query&titles={urllib.parse.quote(title)}&prop=pageimages&pithumbsize=1000&format=json"
...
for pkg in db["packages"]:
    if pkg["title"] == "Jewels of North East":
        img_url = get_wikimedia_image("Sikkim Gangtok landscape")
        if img_url:

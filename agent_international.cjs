...
(async () => {
    const internationalPackages = db.packages.filter(p => p.type === 'International');
    console.log(`Starting Pixabay Agent for ${internationalPackages.length} International packages...`);
    ...
    for (let p of internationalPackages) {
        const pkg_id = p.id;
        ...
        if (p.title.toLowerCase().includes('buddhist')) query = 'Bodhgaya temple';
        ...
        try {
            ...
            const images = await page.evaluate(() => {
                return Array.from(document.querySelectorAll('img'))
                    .map(img => img.src)
                    .filter(src => src.includes('pixabay.com/photo/') || src.includes('pixabay.com/get/'));
            });
    ...
    await browser.close();
    console.log("Agent finished downloading Pixabay hi-res images for International.");
})();

...
(async () => {
    ...
    for (let p of db.packages) {
        if (p.type !== "Domestic") continue;

        const pkg_id = p.id;
        const destPath = path.join(OUT_DIR, `${pkg_id}.jpg`);
        
        // Skip if already downloaded and valid
        // Actually, we want to download new unique images for ALL of them to fix repeats, so let's remove the skip check!
        ...
        try {
            ...
            if (images.length > 0) {
                // Get a random image from the first few to avoid repeats
                let randomIndex = Math.floor(Math.random() * Math.min(images.length, 10));
                let imgUrl = images[randomIndex];
